...
from .models import TipoDocumento, Tipo
from rest_framework import serializers


class TipoSerializers  (serializers.Serializer):
    class Meta:
        model = Tipo
        fields = '__all__'


class TipoDocumentoSerializers (serializers.Serializer):
...
class ArticleSerializer(serializers.ModelSerializer):
    ...
    cover_name = serializers.FileField(
        use_url=False, source='cover', required=True)
    group = serializers.SlugRelatedField(
        slug_field='uuid', queryset=ArticleGroup.objects.all())
    original_group = serializers.SlugRelatedField(
    ...
    class Meta:
        ...
        read_only_fields = ('date_created', 'date_modified',
                            'references', 'uuid', 'cover_name', 'read_only_nullable')
        lookup_field = 'slug'
        extra_kwargs = {
...
class ImageUploadSerializer(serializers.Serializer):
    ...
    image_styles = serializers.ListSerializer(
        child=serializers.ChoiceField(
            choices=['wide', 'tall', 'thumb', 'social']),
